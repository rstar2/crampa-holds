...
const deleteRenderPattern = (client, pattern, callback = noop) => {
	// TODO: 
	const args = createKey(pattern, '*');

	// NOTE - This will be implemented as not-atomic operation
	client.hscan(hashKeyRender, args, function (err, keys) {
		if (err) return callback(err);

		client.hdel(hashKeyRender, keys, function (err, count) {
...
module.exports = function (client) {
	...
	return {
		...
		render: {
			...
			delPages: deleteRenderPattern.bind(null, client, 'page'),
			delBlog: deleteRenderPattern.bind(null, client, 'blog'),
			delGalleries: deleteRenderPattern.bind(null, client, 'gallery'),
