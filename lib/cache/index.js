...
const scanDeleter = (client, hashKey, match) => {
	...
	const scan = (cursor, callback) => {
		client.hscan(hashKey, cursor, 'MATCH', match, 'COUNT', 1, function (err, iteration) {
			if (err) {
				return callback(err, deletedKeys);
			}

			const cursor = +iteration[0];
			// TODO ? in HSCAN the second item in the 
			// returned array of elements contain two elements, a field and a value, for every returned element of the Hash.
			const elements = iteration[1];

			const next = (err, count) => {
				if (err) {
					return callback(err, deletedKeys);
				}
