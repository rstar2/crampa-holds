...
module.exports = function (keystone) {
	...
	return function (req, res, next) {
		...
		res.cache = function (err, response) {
			...
			// TODO: use the expire timeout
			// TODO: store also the content type of the response? or for now just assume all is plain HTML
			cache.set(cacheKey, response, function (err) {
				if (err) debug(`Failed to cache response for request ${req} with key ${cacheKey}`);
				else debug(`Cached response for request ${req} with key ${cacheKey} - expires after ${expire}`);
		...
		res.cache.expire = function (secs) {
			expire = secs;
			return res.cache;
		};
